...
- 新增「设置 → 浮窗 → 仅点击展开」：完整额度、极简组件及菜单栏均可改为点击打开详情，鼠标经过时保持收起。
- 点击展开后，鼠标移出会按设置的延迟自动收起；延迟期间移回可取消收起。完整和极简浮窗也可点击右上角箭头立即收起。
- 点击模式下，应用内反馈不再主动展开面板；系统通知开关保持独立。保留拖动定位、全局显示/隐藏快捷键及重启后的设置记忆。
- 新开关默认关闭，保留已有悬停交互；简体中文、繁体中文和英文说明同步更新。

## 0.2.4 — 2026-09-08

发布标签：`preview-0.2.4`。Universal 2 DMG / ZIP，ad-hoc 签名、未公证。[图文更新说明](docs/releases/0.2.4.md)。

- 修复官方重置后仍显示“本机尚未验证”：标准 Codex 额度从较低已用量归零且周期锚点明显延后时也能识别，不再仅依赖下降 30 个百分点；手动重置次数减少、自然周期刷新及专项额度变化不会据此确认官方自动重置。
- 启动和公告刷新时重放本机保留的额度快照，补回漏识别的重置；确认状态及本机首次检测时间持久保存，公告重新分类不会覆盖同类型的已确认记录。
- 公告支持“Lands around 6pm PST today”这类明确时间；PST / PDT 按原文固定时差、PT 按美国太平洋夏令时换算，以原帖当地日期为基准。已观察到变化时显示本机检测时间；原帖未提供具体时间时直接说明，不再显示矛盾的“公告时已重置：待确认”。

## 0.2.3 — 2026-09-04

发布标签：`preview-0.2.3`。本次提供 Universal 2 DMG / ZIP，采用 ad-hoc 签名，尚未经过 Apple Developer ID 签名与公证。

### Tibo 公告时间与重置类型

- 不再把帖子发布时间或抓取时间直接显示成预计重置时间。优先从 X 帖子 Snowflake ID 还原精确发布时间，再根据该条原文中的相对时间表达计算预计事件时间；第三方镜像的“几小时前”等粗粒度标签只作为兜底。
- 时间规则不固定为 3 小时：支持分钟、小时、天，数字和常见英文数字词，以及 `about`、`within`、`~` 等表述。没有明确时间时保持“待确认”，不会自行猜测。
- 到期/有效期时长不会被当成发放时间。公开样本 `07:12:09 发布 + 约 3 小时` 已验证为 `约 10:12:09 发放`，延迟抓取不会改变结果。
- 明确区分“手动重置卡”和“官方自动重置”：前者到账后需要用户自行使用，界面显示“预计发放”；后者不需要手动卡片，界面显示“预计官方自动重置”或“公告时已由官方自动重置”。浮层、活动记录、应用内反馈和系统通知同步使用对应文案。
- 普通全量、Address Sanitizer 与 Thread Sanitizer 均为 215 项通过；重置分类定向 9 项、帖子时间定向 3 项及 Release 警告即错误构建通过；真实运行缓存已核对精确发布时间、事件类型和预计发放时间。
