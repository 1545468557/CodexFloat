...
![功能示意：识别额度重置、保留本机确认结果与检测时间](docs/Images/codex-float-0.2.4-feature.png)
...
当前主分支新增「设置 → 浮窗 → 仅点击展开」，适用于完整额度、极简进度条和菜单栏额度。开启后，鼠标经过不会弹出详情；点击后才展开，移开鼠标后自动收起。可在「鼠标移出后收起」中选择立即、0.6 秒、1 秒或 2 秒，延迟期间移回面板会取消收起。完整和极简浮窗右上角的箭头也可立即收起详情。

该开关默认关闭，开启后的选择会在重启后保留。此功能尚未包含在上方的 0.2.4 预览安装包中，可按下方说明从当前源码构建。
